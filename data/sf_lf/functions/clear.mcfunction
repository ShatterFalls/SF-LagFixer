...
## Filter
### Blacklists
execute as @e[type=item] run function sf_lf:debug/blacklist/item
...
## Clear Notify
execute if score LF.Cleared SF.LagFixer matches 0 run tellraw @a [{"text": "[","color": "gray"},{"text": "LF","color": "green","hoverEvent": {"action": "show_text","value": [{"text": "[","color": "gray"},{"text": "SF","color": "dark_purple"},{"text": "]","color": "gray"},{"text": "LagFixer - Statistics","color": "gold"}]},"clickEvent": {"action": "run_command","value": "/function sf_lf:settings/statistics"}},{"text":"] ","color": "gray"},{"text":"Nothing was cleared!","color":"gold"}]
